...
function checkIsRuning(){
  ...
  if [[ ${projectPid} != "" ]]
    then
      ...
      while true 
      do
          if [[ $(ps aux | grep $1 | grep -v "grep" | tr -s ' '| cut -d ' ' -f 2) == "" ]]
          then
              break
          fi
          sleep 1s
...
function startJar(){
  ...
  #启动项目
  echo "nohup /root/home/tools/jdk17/bin/java -jar -Dspring.profiles.active=${PROJECT_ACTIVE_PROFILE} ${mvName}  > /dev/null 2>&1 &"
  nohup /root/home/tools/jdk17/bin/java -jar -Dspring.profiles.active=${PROJECT_ACTIVE_PROFILE} ${mvName} > /dev/null 2>&1 &
  ...
  echo "启动成功 (匹配到启动成功日志标识符 \"${LOG_SUCCESS_END_MARK}\")"
}
